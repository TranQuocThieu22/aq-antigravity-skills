...
4. **Use `aq-core-framework` for API UI Components**: When using UI components that trigger API calls directly (such as Delete buttons in tables), always import them from `@aq-fe/aq-core-framework` (e.g., `CustomButtonDelete`, `CustomButtonDeleteList`). Do NOT use the legacy/standalone versions from `core-ui` as they lack the newer integrated API logic.
