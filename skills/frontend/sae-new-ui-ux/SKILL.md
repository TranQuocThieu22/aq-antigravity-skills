...
13. **Do Not Manually Add STT (Số Thứ Tự) Column**: `CustomTanstackTable` automatically generates and prepends an "STT" (sequence number) column by default. Manually adding an accessor for "stt" or a column header for "STT" will cause React duplicate key errors (`Encountered two children with the same key, 'stt'`). Simply omit the STT column from your `columns` definition when using `CustomTanstackTable`.
